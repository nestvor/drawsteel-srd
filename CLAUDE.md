...
- **Hand-authored data**: `src/data/json/monster-basics.json` is NOT generated by `extract.ts`
  (forgesteel has no director "Monster Basics" rules). It's a `single`-style glossary rendered in
  authored (chapter) order. It's rebuilt from a local, gitignored `monster_basics.txt` extract via
  `node scripts/build-monster-basics.mjs` (splits at section headings → cleans extraction noise →
  reformats data blocks into Markdown tables / a styled stat block / icon legend → inserts the
  original SVG diagrams → re-flows hard-wrapped prose into paragraphs + `###` sub-headings). Re-run it
  after re-extracting. Art from the books must NOT be reproduced (Creator License) — the diagrams are
  original SVG recreations of the mechanics only.
