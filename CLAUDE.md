...
- **Hand-authored data**: `src/data/json/monster-basics.json` is NOT generated by `extract.ts`
  (forgesteel has no director "Monster Basics" rules). It's maintained by hand; `extract` leaves it
  untouched. It renders as a `single`-style glossary in authored (chapter) order. Art from the books
  must not be reproduced (Creator License); use original diagrams only.
